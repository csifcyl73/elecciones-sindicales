...
export default function InterventorNacionalDashboard() {
  ...
  const loadAllMesas = async () => {
    setLoading(true);
    try {
      // Necesitamos una API que devuelva todas las mesas sin filtrar por usuario
      // Usaremos la de unidades y luego expandiremos a mesas
      const res = await fetch('/api/admin/unidades');
      const unidades = await res.json();
      
      // Extraer todas las mesas de todas las unidades activas
      const allMesas: any[] = [];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div>
            ...
            <h1 className="text-4xl md:text-6xl font-black tracking-tighter uppercase leading-none">
              Intervención <br />
              <span className="text-black bg-emerald-400 px-4 py-2 inline-block -skew-x-6 mt-2">Nacional</span>
            </h1>
          </div>
