...
export default function VisualizarEleccionesPage() {
  ...
  const hasFilters = filterProvincias.length > 0 || filterSectores.length > 0 || filterUnidades.length > 0 || filterOrganos.length > 0 || filterAnyos.length > 0;
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="mb-10 space-y-3">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {hasFilters && (
              <button
                ...
                className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-white/30 hover:text-rose-400 transition-colors"
              >
          ...
          <div className="flex flex-wrap gap-2">
            {/* Helper para renderizar cada dropdown */}
            {[
              ...
              <div key={key} className="relative">
                ...
                >
                  ...
                  {label}
                  {selected.length > 0 && (
                    <span className="bg-rose-500/30 text-rose-300 rounded-full w-4 h-4 flex items-center justify-center text-[8px]">
                      {selected.length}
                    </span>
                  )}
                      ...
                      {opts.map((opt: string) => (
                        ...
                        >
                          <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-all ${
                            selected.includes(opt) ? 'bg-rose-500 border-rose-500' : 'border-white/20'
                          }`}>
          ...
          {hasFilters && (
            <div className="flex flex-wrap gap-2 pt-1">
              {[...filterProvincias.map(v => ({ v, label: 'Provincia', setter: setFilterProvincias })),
                ...
                <span
                  key={`${label}-${v}`}
                  className="inline-flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-300 text-[9px] font-black uppercase tracking-widest"
                >
                  <span className="text-rose-500/50">{label}:</span> {v}
                  <button
                    onClick={() => toggleFilter(setter, v)}
                    className="w-4 h-4 rounded-full bg-rose-500/20 hover:bg-rose-500/40 flex items-center justify-center transition-all"
                  >
