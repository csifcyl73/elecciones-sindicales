...
function ConfigurarEleccionesSPA() {
  ...
  const [comunidadAdmin, setComunidadAdmin] = useState<string>('');
  ...
  const loadMasters = async () => {
    ...
    try {
      ...
      const userCom = session?.user?.user_metadata?.comunidad || '';
      setComunidadAdmin(userCom);

      const [provRes, sectRes, organRes, unityRes, muniRes, intRes, sindRes, procRes] = await Promise.all([
        supabase.from('provincias').select('*, ccaa(nombre)').order('nombre'),
      ...
      if (provRes.data) {
        ...
        setProvincias([...provRes.data].sort((a: any, b: any) => a.nombre.localeCompare(b.nombre, 'es', { sensitivity: 'base' })));
      }
