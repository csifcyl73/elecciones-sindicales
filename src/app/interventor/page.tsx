...
import { Mail, Inbox, FileText, Users, ArrowLeft, Loader2, UserCheck } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';
import { useRouter } from 'next/navigation';
import { useGoogleReCaptcha } from 'react-google-recaptcha-v3';

export default function InterventorLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (executeRecaptcha) {
      try {
        const token = await executeRecaptcha('login_interventor');
    ...
    // Verificar que el usuario tiene el rol correcto en su metadata
    const role = user?.user_metadata?.role;
    if (role !== 'interventor') {
      await supabase.auth.signOut();
      setError('Acceso denegado. Esta área es exclusiva para Interventores autorizados.');
      setLoading(false);
      return;
    }

    router.push('/interventor/dashboard');
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 flex-col items-center justify-center relative overflow-hidden">
      ...
      {/* Marcas de agua de fondo (Consistente con Admin) */}
      ...
      <div className="relative z-10 mb-6">
        <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-100 border border-emerald-200 text-emerald-700 text-sm font-semibold">
          <UserCheck className="w-4 h-4" />
          Acceso de Interventor
        </span>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-white/60 backdrop-blur-lg rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 flex flex-col items-center">
        ...
        <form className="w-full space-y-6" onSubmit={handleLogin}>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-gray-700 ml-1">Email de Interventor</label>
            <input
              type="email"
              placeholder="Introduce tu email institucional..."
              value={email}
